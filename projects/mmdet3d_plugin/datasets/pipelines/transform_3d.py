...
class PadMultiViewImage(object):
    ...
    def _pad_img(self, results):
        ...
        if self.size is not None:
            ...
            if 'seg_gt' in results.keys():
                padded_seg_gt = [mmcv.impad(
                img, shape=self.size, pad_val=255) for img in results['seg_gt']]
            ...
            if 'seg_gt' in results.keys():
                padded_seg_gt = [mmcv.impad_to_multiple(
                    img, self.size_divisor, pad_val=255) for img in results['seg_gt']]

        results['ori_shape'] = [img.shape for img in results['img']]
        results['img'] = padded_img
        if "dpt" in results.keys():
            results['dpt'] = padded_dpt
        if 'seg_gt' in results.keys():
            results['seg_gt'] = padded_seg_gt
...
class RandomScaleImageMultiViewImage(object):
    ...
    def __call__(self, results):
        ...
        if "dpt" in results.keys():
            results["dpt"] = [mmcv.imresize(img, (x_size[idx], y_size[idx]), return_scale=False, interpolation="nearest") for idx, img in enumerate(results['dpt'])] 
        if "seg_gt" in results.keys():
            results["seg_gt"] = [mmcv.imresize(img, (x_size[idx], y_size[idx]), return_scale=False, interpolation="nearest") for idx, img in enumerate(results['seg_gt'])] 
